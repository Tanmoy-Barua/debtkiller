...
| `npm run health` | Audit, unit test, build, preview, and browser smoke test |
| `npm test` | Theme unit checks |
| `npm run test:e2e-theme` | Browser smoke (Playwright; app must be running) |

The app health workflow runs `npm run health` automatically at **07:00 and 19:00 UTC** every day, and can also be started manually from GitHub Actions.
